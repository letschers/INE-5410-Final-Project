...
typedef enum god {
    BALDR,
    LOKI, /* primeiro e segundo deseus são rivais. */
    VALI,
    HODER, /* terceiro e quarto deuses são rivais.  */
    FRIGG,
    JORD, /* quinto e sexto deuses são rivais.     */
...
typedef struct valhalla {
    unsigned int
        prayers[NUMBER_OF_GODS]; /* Controla o número de preces por deus. */

    pthread_mutex_t
        mutex; /* Protege o contador de preces (acesso concorrente). */
} valhalla_t;
